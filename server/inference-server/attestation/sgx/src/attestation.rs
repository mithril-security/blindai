...
const ATTESTATION_VALIDITY_SECS: u64 = 3600; 
...
impl AttestedTlsConfig {
    pub fn new(attestation_config: &AttestationConfig) -> Result<AttestedTlsConfig> {
        trace!("let key_pair = key::NistP256KeyPair::new()?;");
        let key_pair = key::NistP256KeyPair::new()?;
        trace!(" let report = match attestation_config ");
        ...
        trace!("let extension = serde_json::to_vec(&report)?;");
        let extension = serde_json::to_vec(&report)?;
        trace!("let cert = key_pair.create_cert_with_extension(CERT_ISSUER, CERT_SUBJECT, &extension);");
        let cert = key_pair.create_cert_with_extension(CERT_ISSUER, CERT_SUBJECT, &extension);
        trace!("let private_key = key_pair.private_key_into_der();");
        let private_key = key_pair.private_key_into_der();
        trace!("        let time = SystemTime::now();");
