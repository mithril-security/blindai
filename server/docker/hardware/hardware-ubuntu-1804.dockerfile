...
RUN apt-get update && apt-get install -q -y \
    ...
    apt-transport-https software-properties-common systemd net-tools mlocate zip && \
...
RUN add-apt-repository "deb https://download.01.org/intel-sgx/sgx_repo/ubuntu $CODENAME main" && \
    ...
    apt-get install -y \
        ...
        libsgx-dcap-pccs=1.3.101.3-bionic1 \
...
RUN make -C SGXDataCenterAttestationPrimitives/tools/PCKCertSelection/
RUN mkdir -p SGXDataCenterAttestationPrimitives/QuoteGeneration/pccs/lib/
RUN cp SGXDataCenterAttestationPrimitives/tools/PCKCertSelection/out/libPCKCertSelection.so SGXDataCenterAttestationPrimitives/QuoteGeneration/pccs/lib/
RUN cp -R SGXDataCenterAttestationPrimitives/QuoteGeneration/pccs/ /opt/intel/sgx-dcap-pccs
RUN rm -rf SGXDataCenterAttestationPrimitives

RUN npm install esm
RUN npm install pm2
RUN npm update pm2

ADD docker/hardware/pccs.sh /root
RUN /root/pccs.sh
...
RUN sed -i 's/#USE_SECURE_CERT=FALSE/USE_SECURE_CERT=FALSE/g' /etc/sgx_default_qcnl.conf

COPY bin/ /root/
ADD docker/hardware/start.sh /root
