...
pub fn init_sgx_quote() -> Result<(sgx_att_key_id_t, sgx_target_info_t)> {
    debug!("init_quote");
...
pub fn create_sgx_isv_enclave_report(
    ...
    report_data.d[..32].copy_from_slice(hash);
...
pub fn get_sgx_quote(ak_id: &sgx_att_key_id_t, report: sgx_report_t) -> Result<Vec<u8>> {
    let mut rt = sgx_status_t::SGX_ERROR_UNEXPECTED;
...
#[cfg(all(feature = "enclave_unit_test", target_env = "sgx"))]
pub mod tests {
    use super::*;
    use crate::key;
