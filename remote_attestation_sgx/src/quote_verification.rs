use std::{ffi::{
    ...
    ptr,
    slice,str};
use anyhow::{anyhow, ensure, bail, Result, Context, Error};
use dcap_ql::{
    quote::{Quote, Quote3SignatureEcdsaP256, Qe3CertDataPckCertChain},
     Quote3Error};
use x509_parser::{oid_registry::Oid, prelude::X509Certificate, prelude::FromDer};
use der_parser::error::{self, BerError, BerResult};
use der_parser::nom::combinator::map;
use der_parser::der::{parse_der_octetstring, parse_der_oid, parse_der_sequence_defined_g, parse_der_sequence_of_v};
use serde::{Serialize, Deserialize};
// use aesm_client::Error;
...
impl VerificationContext {
    ...
    pub fn init(quote: &[u8]) -> VerificationResult<Self> {
        let quote_slice = quote.clone();
        let (fmspc, ca_from_quote, pck_certificate, pck_signing_chain) = get_fmspc_ca_from_quote(quote)?;
        Ok(Self{
            quote_slice: quote_slice.to_vec(), 
            fmspc: fmspc, 
            ca_from_quote: ca_from_quote, 
    ...
    pub fn get_quote_verification_collateral(&self) -> Result<SgxQlQveCollateral, VerificationError> {
        Ok(sgx_get_quote_verification_collateral(&self.fmspc, &self.ca_from_quote)?)
    }
...
extern "C" {
    pub fn sgx_ql_get_quote_verification_collateral(fmspc: *const uint8_t, 
                                                    fmspc_size: uint16_t, 
                                                    pck_ra: *const c_char, 
                                                    pp_quote_collateral: *mut *mut sgx_ql_qve_collateral_t) -> Quote3Error;
    pub fn sgx_ql_free_quote_verification_collateral(p_quote_collateral: *const sgx_ql_qve_collateral_t) -> Quote3Error;                                                
}
...
pub fn sgx_get_quote_verification_collateral(fmspc: &[u8; 6], ca_from_quote: &CString) 
...
pub fn get_fmspc_ca_from_quote(quote: &[u8]) -> Result<([u8; 6], CString, String, String)> {
    ...
    let cert_chain = certification_data.unwrap().certs;

    let mut cert_chain_str = String::from("");
    for cert in 0..cert_chain.len() {
        cert_chain_str.push_str(cert_chain[cert].borrow());
    }
    let pems = pem::parse_many(cert_chain_str).unwrap();
    ensure!(
        pems.len() == 3, 
